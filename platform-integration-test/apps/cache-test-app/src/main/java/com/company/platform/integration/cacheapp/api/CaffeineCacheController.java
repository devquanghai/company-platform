...
public class CaffeineCacheController {
    private static final String CACHE_NAME = "demo-caffeine";
    private final NamedCacheTestService caches;

    public CaffeineCacheController(NamedCacheTestService caches) {
        this.caches = caches;
    }
    ...
    public CacheMutationResponse put(
        ...
        return caches.put(CACHE_NAME, "CAFFEINE", key, request.value());
    }

    @DeleteMapping("/{key}")
    public CacheMutationResponse evict(@PathVariable @Size(max = 128) String key) {
        return caches.evict(CACHE_NAME, "CAFFEINE", key);
    }
