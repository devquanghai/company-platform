...
public abstract class BaseKafkaConsumer<T> {
    ...
    protected BaseKafkaConsumer(String subscription, Class<T> payloadType) {
        this(subscription, null, payloadType);
    }

    protected BaseKafkaConsumer(
        String subscription, String handlerId, Class<T> payloadType
    ) {
    ...
    public final MessageHandlingResult handleMessage(
        Object payload, MessageContext context
    ) {
    ...
    private static Method handlerMethod() {
        try {
            return BaseKafkaConsumer.class.getMethod(
